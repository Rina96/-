...
class AlfaCrmManager:
    ...
    async def get_customer_by_phone(self, phone: str) -> Optional[Dict[str, Any]]:
        """Safe lead lookup with 5s timeout. Using full numeric ID for accuracy."""
        try:
            headers = await self.get_headers()
            url = f"{self.base_url}/{self.BRANCH_ID}/customer/index"
            clean_phone = "".join(filter(str.isdigit, phone)) # Use full digits for Kazakhstan (e.g. 7701...)
            async with httpx.AsyncClient(verify=False) as client:
                r = await client.post(
                    url, headers=headers, json={"phone": clean_phone}, timeout=5.0
                )
                logger.debug(f"🔍 CRM Lookup for {clean_phone}: Status {r.status_code}")
                if r.status_code == 200:
            ...
            if name not in ("WA Lead", "WhatsApp Lead", "") and \
               current_name in ("WA Lead", "WhatsApp Lead", "", "Lead"):
                ...
                try:
                    ...
                    async with httpx.AsyncClient(verify=False) as client:
                        r = await client.post(url, headers=headers, json={"name": name}, timeout=5.0)
                        logger.debug(f"✅ CRM Name Update Status: {r.status_code}")
                except Exception as e:
